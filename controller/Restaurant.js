...
const getRestaurantDetails = async (req, res) => {
  try {
    ...
    if (restaurantDetails) {
      const food = await Food.find({ restaurant: restaurantDetails._id })
        .populate("ingredients")
        .populate("restaurant", { password: 0 });
...
const getRestaurantsByGroup = async (req, res) => {
  try {
    ...
    const goldrestaurants = await Restaurant.find({
      category: goldcategory._id,
    }).populate("category");
    const silvercategory = await RestoCategory.findOne({ title: "Silver" });
    const silverrestaurants = await Restaurant.find({
      category: silvercategory._id,
    }).populate("category");
    const platinumcategory = await RestoCategory.findOne({ title: "Platinum" });
    const platinumrestaurants = await Restaurant.find({
      category: platinumcategory._id,
    }).populate("category");
    if (
      goldcategory &&
      goldrestaurants &&
